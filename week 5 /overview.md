# Overview
In this workshop, we will demonstrate how to leverage both relational (Amazon Aurora) and non-relational databases (Amazon DynamoDB) that are purpose-built to handle the specific needs of an application. 
For illustrative purpose, we will leverage a relational schema used by a taxi application.  

The figure below depicts the high level deployment architecture.
![week5-lab](https://github.com/xhelma/12weekawsworkshopchallenge/assets/97184575/6ca5fe95-147e-4a83-a3f6-fff3a3e539af)


# Prerequisites

Before starting the first lab, the CloudFormation sample template `Oracle_DynamoDB_Aurora_PostgreSQL_DMS` must be deployed. This CloudFormation template creates an Oracle and Aurora 
PostgreSQL RDS instances in a VPC which can be used to test the database migration using AWS DMS service. It also creates a DynamoDB table and a Cloud9 environment.

<details>
  <summary>Oracle_DynamoDB_Aurora_PostgreSQL_DMS</summary>
